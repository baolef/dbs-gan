...
if __name__ == '__main__':
    ...
    writer=Writer(opt)
    # writer.add_models(model.get_networks())
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            total_loss+=model.get_early_stop_loss()

            if opt.display_freq>0 and total_iters % opt.display_freq == 0:   # display images on visdom and save images to a HTML file
        ...
        if total_loss<lowest_loss:
            ...
            model.compute_visuals()
            results = model.get_current_visuals()
            visualizer.save_imgs(results)

        early_stopping(total_loss)
        if early_stopping.early_stop:
            print('Early Stop!')
            print('saving the model at the end of epoch %d, iters %d' % (epoch, total_iters))
            model.save_networks('stop')
            model.save_networks(epoch)
            break

        model.update_learning_rate()    # call lr_scheduler.step() after optimizer.step()
        writer.add_lr(model.get_learning_rate(), total_iters)
